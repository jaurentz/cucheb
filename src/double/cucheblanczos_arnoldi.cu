...
int cucheblanczos_arnoldi(cuchebmatrix* ccm, cucheblanczos* ccl){
  ...
  for(int ii=0; ii < nvecs; ii++){
    ...
    // orthogonalize
    cublasDgemv(ccl->handle, CUBLAS_OP_T, n, (ii+1), &one, &dvecs[0], n, 
                &dvecs[(ii+1)*n], 1, &zero, &dschurvecs[ii*nvecs], 1);
    cublasDgemv(ccl->handle, CUBLAS_OP_N, n, (ii+1), &mone, &dvecs[0], n, 
    ...
    cublasDaxpy(ccl->handle, (ii+1), &one, &dtemp[0], 1, 
                &dschurvecs[ii*nvecs], 1);

    // normalize
    cublasDnrm2(ccl->handle, n, &dvecs[(ii+1)*n], 1, &sdiag[ii]);
    scl = 1.0/sdiag[ii];
    cublasDscal(ccl->handle, n, &scl, &dvecs[(ii+1)*n], 1);
