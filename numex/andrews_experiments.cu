...
int main(){
  ...
  // lower bounds
  double lb[nummats] = { 4.0 };

  // upper bounds
  double ub[nummats] = { 5.0 };

  // number of trials
  const int numtrials = 3;

  // matrix names
  string matnames[nummats*numtrials] = { "Andrews",
                                         "Andrews",
                                         "Andrews" };

  // matrix names
  int degrees[nummats][numtrials] = { {100,200,-1} };


  // output file
  string ofile("./numex/andrews_data.txt" );
